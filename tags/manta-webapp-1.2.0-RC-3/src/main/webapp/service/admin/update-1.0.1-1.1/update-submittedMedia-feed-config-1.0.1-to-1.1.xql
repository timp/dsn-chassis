...
<atom:feed xmlns:atom="http://www.w3.org/2005/Atom" xmlns:atombeat="http://purl.org/atombeat/xmlns" atombeat:enable-tombstones="true">
    <atom:id>http://localhost:8081/repository/service/content/media/submitted/XYNCE</atom:id>
    <atom:updated>2011-01-10T10:17:34.576Z</atom:updated>
    <atom:link rel="self" href="http://localhost:8081/repository/service/content/media/submitted/XYNCE" type="application/atom+xml;type=feed"/>
    <atom:link rel="edit" href="http://localhost:8081/repository/service/content/media/submitted/XYNCE" type="application/atom+xml;type=feed"/>
    <atom:author>
        <atom:email>colin@example.org</atom:email>
    </atom:author>
    <atom:title type="text">Submitted Media for Study XYNCE</atom:title>
    ...
    <app:collection xmlns:app="http://www.w3.org/2007/app" href="http://localhost:8081/repository/service/content/media/submitted/XYNCE">
        <atom:title type="text">Submitted Media for Study XYNCE</atom:title>
    </app:collection>
</atom:feed>
};
...
{
    let $submittedMedia-feed-configs := local:get-content()
    ...
    return
        ...
        <html>
            <head>
                <title>Data Migration - Submitted Media Feed Config v1.0.1 to v1.1</title>
            </head>
            <body>
                <h1>Data Migration - Submitted Media Feed Config v1.0.1 to v1.1</h1>
                <p>This script should change the submittedMedia feed configs so that...</p>
                <ul>
...
declare function local:get-content() as item()* {
  let $testing := request:get-parameter("testing", "no")
    let $content := if ($testing = "no") then
        let $ret := for $child-collection-name in xmldb:get-child-collections( "/db/atombeat/content/media/submitted/" )
                    return xmldb:document(concat("/db/atombeat/content/media/submitted/", $child-collection-name, "/.feed"))
        return $ret
    else
        let $ret := xmldb:xcollection( 'test-submittedMedia-feed-config' )/atom:feed (: not recursive :)
        return $ret
     
   return $content
};

declare function local:do-modifications($submittedMedia-feed-configs-v1-0-1) as item()*
{
    let $new := local:modify-nodes($submittedMedia-feed-configs-v1-0-1)
...
{
        let $descriptor-doc-db-path := xmldb:store( 'test-submittedMedia-feed-config' , ".feed" , $testdata , $CONSTANT:MEDIA-TYPE-XML )
...
declare function local:modify-nodes($submittedMedia-feed-configs-v1-0-1) as item()*
{
   let $submittedMedia-feed-configs-v1-1 := 
        for $v1-0-1 in $submittedMedia-feed-configs-v1-0-1

			return update insert <atombeat:config context="feed"><atombeat:param name="match-rels" value="http://purl.org/atombeat/rel/security-descriptor"/></atombeat:config> following $v1-0-1//atombeat:config[@context = 'entry' and atombeat:param/@name = 'match-rels' and atombeat:param/@value = '*']

    return $submittedMedia-feed-configs-v1-1
...
let $collection := xmldb:create-collection("xmldb:exist:///db", "test-submittedMedia-feed-config"),
...
return 
    ...
    else common-protocol:do-method-not-allowed( "/admin/update-submittedMedia-feed-config-1.0.1-to-1.1.xql" ,"/admin/update-submittedMedia-feed-config-1.0.1-to-1.1.xql" , ( "GET" , "POST" ) )
