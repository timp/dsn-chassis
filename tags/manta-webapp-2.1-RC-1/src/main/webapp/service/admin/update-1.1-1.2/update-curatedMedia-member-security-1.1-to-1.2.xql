...
declare function local:content($content) as item()*
{
    ...
    return
        ...
        <html>
            <head>
                <title>Data Migration - Curated Media Member Security v1.1 to v1.2</title>
            </head>
            <body>
                <h1>Data Migration - Curated Media Member Security v1.1 to v1.2</h1>
                <p>This script should change the curatedMedia ACL so that...</p>
                <ul>
                    <li>Curators can add curatedMedia to any study. {count($curatedMedia-member-securities//atombeat:ace[atombeat:recipient/@type = 'role' and atombeat:recipient = 'ROLE_CHASSIS_CURATOR' and atombeat:permission = 'CREATE_MEDIA'])}
                    </li>
                    <li>Curators can update curatedMedia in any study. {count($curatedMedia-member-securities//atombeat:ace[atombeat:recipient/@type = 'role' and atombeat:recipient = 'ROLE_CHASSIS_CURATOR' and atombeat:permission = 'UPDATE_MEMBER'])}
                    </li>
                </ul>
                <p>
...
declare function local:modify-nodes($curatedMedia-member-securities-old) as item()*
{
   let $curatedMedia-member-securities-new := 
        for $old in $curatedMedia-member-securities-old
            ...
            let $ins1 := if (not(exists($old//atombeat:ace[atombeat:recipient/@type = 'role' and atombeat:recipient = 'ROLE_CHASSIS_CURATOR' and atombeat:permission = 'CREATE_MEDIA']))) then
            update insert <atombeat:ace><atombeat:type>ALLOW</atombeat:type><atombeat:recipient type="role">ROLE_CHASSIS_CURATOR</atombeat:recipient><atombeat:permission>CREATE_MEDIA</atombeat:permission></atombeat:ace> following $old//atombeat:ace[atombeat:recipient/@type = 'role' and atombeat:recipient = 'ROLE_CHASSIS_CURATOR' and atombeat:permission = 'RETRIEVE_MEDIA']
            else ()
            let $ins2 := if (not(exists($old//atombeat:ace[atombeat:recipient/@type = 'role' and atombeat:recipient = 'ROLE_CHASSIS_CURATOR' and atombeat:permission = 'UPDATE_MEMBER']))) then
            update insert <atombeat:ace><atombeat:type>ALLOW</atombeat:type><atombeat:recipient type="role">ROLE_CHASSIS_CURATOR</atombeat:recipient><atombeat:permission>UPDATE_MEMBER</atombeat:permission></atombeat:ace> following $old//atombeat:ace[atombeat:recipient/@type = 'role' and atombeat:recipient = 'ROLE_CHASSIS_CURATOR' and atombeat:permission = 'RETRIEVE_MEDIA']
            else ()
        return $ins2
...
declare function local:check-changes() as item() *{
 ...
 let $ret := for $m in $all 
         ...
         let $out := if (count($m//atombeat:ace[atombeat:recipient/@type = 'role' and atombeat:recipient = 'ROLE_CHASSIS_CURATOR' and atombeat:permission = 'CREATE_MEDIA']) = 0) then 
                        let $msg := "Failed to insert //atombeat:ace[atombeat:recipient/@type = 'role' and atombeat:recipient = 'ROLE_CHASSIS_CURATOR' and atombeat:permission = 'CREATE_MEDIA']"
                        return $msg
                     else
                        let $msg := "Inserted //atombeat:ace[atombeat:recipient/@type = 'role' and atombeat:recipient = 'ROLE_CHASSIS_CURATOR' and atombeat:permission = 'CREATE_MEDIA']"
                        return $msg
         ...
         let $out2 := if (count($m//atombeat:ace[atombeat:recipient/@type = 'role' and atombeat:recipient = 'ROLE_CHASSIS_CURATOR' and atombeat:permission = 'UPDATE_MEMBER']) = 0) then 
                        let $msg := "Failed to insert //atombeat:ace[atombeat:recipient/@type = 'role' and atombeat:recipient = 'ROLE_CHASSIS_CURATOR' and atombeat:permission = 'UPDATE_MEMBER']"
                        return $msg
                     else
                        let $msg := "Inserted //atombeat:ace[atombeat:recipient/@type = 'role' and atombeat:recipient = 'ROLE_CHASSIS_CURATOR' and atombeat:permission = 'UPDATE_MEMBER']"
                        return $msg
