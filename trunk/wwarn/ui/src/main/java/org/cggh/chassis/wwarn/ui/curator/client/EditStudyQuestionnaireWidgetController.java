 ...
 *
 */
public class EditStudyQuestionnaireWidgetController {

	
	
	
	private Log log = LogFactory.getLog(EditStudyQuestionnaireWidgetController.class);
	
	
	
	
	private EditStudyQuestionnaireWidget owner;
	private EditStudyQuestionnaireWidgetModel model;


	
	
	public EditStudyQuestionnaireWidgetController(EditStudyQuestionnaireWidget owner, EditStudyQuestionnaireWidgetModel model) {
		this.owner = owner;
		this.model = model;
	}
	...
	private Deferred<Element> retrieveSubmission() {
		...
		model.status.set(EditStudyQuestionnaireWidgetModel.STATUS_RETRIEVE_SUBMISSION_PENDING);
...
//				model.studyEntryElement.set(studyEntryElement);
				...
				model.status.set(EditStudyQuestionnaireWidgetModel.STATUS_SUBMISSION_NOT_FOUND);
...
*/
	...
	public Deferred<Element> retrieveStudy() {
		...
		model.status.set(EditStudyQuestionnaireWidgetModel.STATUS_RETRIEVE_STUDY_PENDING);
	...
	public final Function<Element, Element> retrieveStudyCallback = new Function<Element, Element>() {
		...
		public Element apply(Element in) {
			...
			model.status.set(EditStudyQuestionnaireWidgetModel.STATUS_READY_FOR_INTERACTION);
	...
	public void saveStudy() {
		...
		model.status.set(EditStudyQuestionnaireWidgetModel.STATUS_SAVE_STUDY_PENDING);
