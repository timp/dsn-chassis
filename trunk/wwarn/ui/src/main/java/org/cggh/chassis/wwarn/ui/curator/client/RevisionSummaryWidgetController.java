 ...
 *
 */
public class RevisionSummaryWidgetController {

	
	
	
	private Log log = LogFactory.getLog(RevisionSummaryWidgetController.class);
	
	
	
	
	private RevisionSummaryWidget owner;
	private RevisionSummaryWidgetModel model;


	
	
	public RevisionSummaryWidgetController(RevisionSummaryWidget owner, RevisionSummaryWidgetModel model) {
		this.owner = owner;
		this.model = model;
	}
	...
	private Deferred<Element> retrieveSubmission() {
		...
		model.status.set(RevisionSummaryWidgetModel.STATUS_RETRIEVE_SUBMISSION_PENDING);
...
//				model.studyEntryElement.set(studyEntryElement);
				...
				model.status.set(RevisionSummaryWidgetModel.STATUS_SUBMISSION_NOT_FOUND);
...
*/
	...
	public Deferred<Element> retrieveStudy() {
		...
		model.status.set(RevisionSummaryWidgetModel.STATUS_RETRIEVE_STUDY_PENDING);
	...
	public final Function<Element, Element> retrieveStudyCallback = new Function<Element, Element>() {
		...
		public Element apply(Element in) {
			...
			model.status.set(RevisionSummaryWidgetModel.STATUS_READY_FOR_INTERACTION);
	...
	public void saveStudy() {
		...
		model.status.set(RevisionSummaryWidgetModel.STATUS_SAVE_STUDY_PENDING);
