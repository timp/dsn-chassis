...
rm -rf ${COPY_DIR}
mkdir ${COPY_DIR}
for i in `cat files-to-copy.txt`
do
	test -f submitted/$i.xml && mv submitted/$i.xml ${COPY_DIR}
	test -f curated/$i.xml && mv curated/$i.xml ${COPY_DIR}
done
...
do
	...
	test -f files/${NAME} || curl --verbose -k -b ${COOKIES} -o files/${NAME} ${URL}
done
