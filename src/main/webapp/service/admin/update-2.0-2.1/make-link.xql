...
{
    let $x := atomdb:collection-available('/link')
    return
    
        <html>
            <head>
                <title>Creating link collection</title>
            </head>
            <body>
                <h1>Creating link collection</h1>
                
                <p>
                    link collection available: {$x}<br/>
...
declare function local:create-config-collection() as xs:string {
    ...
    let $collection-descriptor := $security-config:link-collection-security-descriptor
