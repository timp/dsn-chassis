 ...
 */
/**
 *  See the documentation at: http://www.orbeon.com/orbeon/doc/reference-processor-api#custom-processors
 *  
 *  The following configuration is also needed:
 *  in WEB-INF/resources/apps/config/properties-local.xml
 *   
 <property as="xs:string"
 name="oxf.schemas.http://chassis.cggh.org/processors/semail"
 value="oxf:/org/cggh/chassis/manta/processor/xml/semail.rng" />

 in WEB-INF/resources/apps/config/custom-processors.xml
 <processors xmlns:chassis="http://chassis.cggh.org/manta/processors">
 <processor name="chassis:semail">
 <class name="org.cggh.chassis.manta.processor.SecureEmailProcessor" />
 </processor>
 </processors>
 */
package org.cggh.chassis.manta.processor;
...
import org.cggh.chassis.manta.util.LookupJNDI;
 ...
 * inline as well as out-of-line attachments.
 * 
 ...
 * TODO:
 * 
 * o revise support of text/html o built-in support for HTML could handle
 * src="cid:*" with part/message ids o support text/xml? or just XHTML? o build
 * message with SAX, not DOM, so streaming of input is possible [not necessarily
 * a big win]
 */
public class SecureEmailProcessor extends ProcessorImpl {

	private static final Logger logger = LoggerFactory
			.createLogger(SecureEmailProcessor.class);

	// Properties for this processor
	public static final String EMAIL_SMTP_HOST = "smtp-host";
	public static final String EMAIL_SMTP_PORT = "smtp-port";
	...
	// Use utf-8 as most email clients support it. This allows us not to have to
	// pick an inferior encoding.
	...
	private static String getProperty(final Element parent, final String name)
		...
		if (ref != null) {
			...
			final String lookupValue = LookupJNDI.<String> getEnvEntry(refVal);
			value = (lookupValue != null) ? lookupValue.trim() : null;
		}
		return (value);
	}
	...
	public void start(PipelineContext pipelineContext) {
		try {
			...
			// dynamically)
			final LocationData locationData = (LocationData) messageElement
					.getData();
			final String dataInputSystemId = locationData.getSystemID();

			// Set SMTP host
			final Properties properties = new Properties();
			final String testSmtpHostProperty = getPropertySet().getString(
			...
			if (testSmtpHostProperty != null) {
				...
				if (host != null && !host.equals("")) {
					...
					if (host == null)
						host = getPropertySet()
								.getString(EMAIL_HOST_DEPRECATED);
					if (host == null)
						throw new OXFException(
								"Could not find SMTP host in configuration or in properties");
					properties.setProperty("mail.smtp.host", host);
			...
			final Session session;
			{
				...
				{
					final Element credentials = messageElement
							.element("credentials");
					if (credentials != null) {
						usernameTrimmed = getProperty(credentials, "username");
						passwordTrimmed = getProperty(credentials, "password");
					} else {
			...
			// Set Cc
			for (final Element ccElement : Dom4jUtils.elements(messageElement,
					"cc")) {
			...
			// Set Bcc
			for (final Element bccElement : Dom4jUtils.elements(messageElement,
					"bcc")) {
			...
			// Set headers if any
			for (final Element headerElement : Dom4jUtils.elements(
					messageElement, "header")) {
				final String headerName = headerElement.element("name")
						.getTextTrim();
				final String headerValue = headerElement.element("value")
						.getTextTrim();

				// NOTE: Use encodeText() in case there are non-ASCII characters
				message.addHeader(headerName, MimeUtility.encodeText(
						headerValue, DEFAULT_CHARACTER_ENCODING, null));
			}

			// Set the email subject
			// The JavaMail spec is badly written and is not clear about whether
			// this needs to be done here. But it
			// seems to use the platform's default charset, which we don't want
			// to deal with. So we preemptively encode.
			// The result is pure ASCII so that setSubject() will not attempt to
			// re-encode it.
			message.setSubject(MimeUtility.encodeText(
					messageElement.element("subject").getStringValue(),
			...
			if (textElement != null) {
				...
				throw new OXFException("Main text or body element not found");// TODO:
																				// location
																				// info
			}
	...
	private void handleBody(PipelineContext pipelineContext,
			String dataInputSystemId, Part parentPart, Element bodyElement)
			throws Exception {
		...
		if (bodyElement.getName().equals("body")) {
			...
			if (multipart != null && !parts.hasNext())
				throw new OXFException(
						"mime-multipart attribute on body element requires part children elements");
			// TODO: Check following lines, which were doing nothing!
			// final String contentTypeFromAttribute =
			...
			final String contentTypeAttribute = NetUtils
					.getContentTypeMediaType(bodyElement
							.attributeValue("content-type"));
			multipart = (contentTypeAttribute != null && contentTypeAttribute
					.startsWith("multipart/")) ? contentTypeAttribute
					.substring("multipart/".length()) : null;
		}
		...
		if (multipart != null) {
			...
			// No multipart, just use the content of the element and add to the
			// current part (which can be the main message)
			handlePart(pipelineContext, dataInputSystemId, parentPart,
					bodyElement);
		}
	}

	private void handlePart(PipelineContext pipelineContext,
			String dataInputSystemId, Part parentPart, Element partOrBodyElement)
			throws Exception {
		final String name = partOrBodyElement.attributeValue("name");
		String contentTypeAttribute = partOrBodyElement
				.attributeValue("content-type");
		final String contentType = NetUtils
				.getContentTypeMediaType(contentTypeAttribute);
		final String charset;
		{
			final String c = NetUtils
		...
		if (src != null) {
			...
			final SAXSource source = getSAXSource(SecureEmailProcessor.this,
			...
			// In the cases of text/html and XML, there must be exactly one root
			// element
			final boolean needsRootElement = "text/html".equals(contentType);// ||
			...
			final Element rootElement = (Element) (needsRootElement ? partOrBodyElement
					.elements().get(0) : partOrBodyElement);
		...
		if (!XMLUtils.isTextOrJSONContentType(contentType)) {
			...
			if (content instanceof FileItem) {
				...
				byte[] data = NetUtils
						.base64StringToByteArray((String) content);
				parentPart.setDataHandler(new DataHandler(
						new SimpleBinaryDataSource(name, contentType, data)));
			}
		} else {
			...
			if (content instanceof FileItem) {
				...
				parentPart.setDataHandler(new DataHandler(
						new SimpleTextDataSource(name, contentTypeWithCharset,
								(String) content)));
			}
		}

		// Set content-disposition header
		String contentDisposition = partOrBodyElement
				.attributeValue("content-disposition");
	...
	private String handleInlinePartContent(Document document, String contentType)
			throws SAXException {
		if ("text/html".equals(contentType)) {
			// Convert XHTML into an HTML String
			StringBuilderWriter writer = new StringBuilderWriter();
			TransformerHandler identity = TransformerUtils
	...
	public static SAXSource getSAXSource(Processor processor,
			PipelineContext pipelineContext, String href, String base,
			String contentType) {
		try {
			// There are two cases:
			// 1. We read the source as SAX
			// o This is required when reading from a processor input; in this
			// case, we behave like
			// the inline case
			// o When reading from another type of URI, the resource could be in
			// theory any type
			// of file.
			// 2. We don't read the source as SAX
			// o It is particularly useful to support this when resources are to
			// be used as binary
			// attachments such as images.
			// o Here, we consider that the source can be XML, text/html,
			// text/*,
			// or binary. We do not handle reading Base64-encoded files. We
			// leverage the URL
			// generator to obtain the content in XML format.
			XMLReader xmlReader;
			{
				String inputName = ProcessorImpl
						.getProcessorInputSchemeInputName(href);
				if (inputName != null) {
					// Resolve to input of current processor
					xmlReader = new ProcessorOutputXMLReader(pipelineContext,
							processor.getInputByName(inputName).getOutput());
				} else {
					// Resolve to regular URI
					Processor urlGenerator = (contentType == null) ? new URLGenerator(
							URLFactory.createURL(base, href))
							: new URLGenerator(
									URLFactory.createURL(base, href),
									contentType, true);
					xmlReader = new ProcessorOutputXMLReader(pipelineContext,
							urlGenerator
									.createOutput(ProcessorImpl.OUTPUT_DATA));
				}
			}
...
//
// part.setDataHandler(new DataHandler(new SimpleTextDataSource(name,
// contentType, content)));
