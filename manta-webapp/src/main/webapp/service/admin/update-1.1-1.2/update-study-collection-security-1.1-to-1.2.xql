...
<atombeat:security-descriptor xmlns:atombeat="http://purl.org/atombeat/xmlns">
    <atombeat:acl><!--
        ...
        <atombeat:ace>
            ...
            give themselves permission to do things we don't want them to do --><!-- TODO: This will become unnecessary in v.1.1.1 -->
        <atombeat:ace>
...
declare function local:content($content) as item()*
{
    ...
    return
        ...
        <html>
            <head>
                <title>Data Migration - Study Collection Security v1.0.1 to v1.2</title>
            </head>
            <body>
                <h1>Data Migration - Study Collection Security v1.0.1 to v1.2</h1>
                <p>This script should change the study collection ACL so that...</p>
                <ul>
                    <li>Users (effectively curators) are allowed to delete members.
                    </li>
                </ul>         
...
declare function local:modify-nodes($study-collection-securities-v1-0-1) as element( atombeat:security-descriptor )*
{
   let $study-collection-securities-v1-1 := 
        for $v1-0-1 in $study-collection-securities-v1-0-1
            ...
            return update delete $v1-0-1//atombeat:ace[atombeat:type/text()='DENY' and atombeat:recipient[@type='user']/text()='*' and atombeat:permission/text()='DELETE_MEMBER']
...
declare function local:check-changes() as item() *{
 ...
 let $ret := for $m in $all 
         ...
         let $out2:= if (count($m//atombeat:ace[atombeat:type/text()='DENY' and atombeat:recipient[@type='user']/text()='*' and ./atombeat:permission/text()='DELETE_MEMBER']) > 0) then 
                        let $msg2 := "Failed to delete DENY * user DELETE_MEMBER "
                        return $msg2
                    else
                        let $msg2 := "Deleted DENY * user DELETE_MEMBER"
                        return $msg2
