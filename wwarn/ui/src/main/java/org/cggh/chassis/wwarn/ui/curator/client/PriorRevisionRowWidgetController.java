 ...
 *
 */
public class PriorRevisionRowWidgetController {

	
	
	
	private Log log = LogFactory.getLog(PriorRevisionRowWidgetController.class);
	
	
	
	
	private PriorRevisionRowWidget owner;
	private PriorRevisionRowWidgetModel model;


	
	
	public PriorRevisionRowWidgetController(PriorRevisionRowWidget owner, PriorRevisionRowWidgetModel model) {
		this.owner = owner;
		this.model = model;
	}
	...
	private Deferred<Element> retrieveSubmission() {
		...
		model.status.set(PriorRevisionRowWidgetModel.STATUS_RETRIEVE_SUBMISSION_PENDING);
...
//				model.studyEntryElement.set(studyEntryElement);
				...
				model.status.set(PriorRevisionRowWidgetModel.STATUS_SUBMISSION_NOT_FOUND);
...
*/
	...
	public Deferred<Element> retrieveStudy() {
		...
		model.status.set(PriorRevisionRowWidgetModel.STATUS_RETRIEVE_STUDY_PENDING);
	...
	public final Function<Element, Element> retrieveStudyCallback = new Function<Element, Element>() {
		...
		public Element apply(Element in) {
			...
			model.status.set(PriorRevisionRowWidgetModel.STATUS_READY_FOR_INTERACTION);
	...
	public void saveStudy() {
		...
		model.status.set(PriorRevisionRowWidgetModel.STATUS_SAVE_STUDY_PENDING);
