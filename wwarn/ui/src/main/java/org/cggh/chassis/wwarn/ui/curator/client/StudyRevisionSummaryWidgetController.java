 ...
 *
 */
public class StudyRevisionSummaryWidgetController {

	
	
	
	private Log log = LogFactory.getLog(StudyRevisionSummaryWidgetController.class);
	
	
	
	
	private StudyRevisionSummaryWidget owner;
	private StudyRevisionSummaryWidgetModel model;


	
	
	public StudyRevisionSummaryWidgetController(StudyRevisionSummaryWidget owner, StudyRevisionSummaryWidgetModel model) {
		this.owner = owner;
		this.model = model;
	}
	...
	private Deferred<Element> retrieveSubmission() {
		...
		model.status.set(StudyRevisionSummaryWidgetModel.STATUS_RETRIEVE_SUBMISSION_PENDING);
...
//				model.studyEntryElement.set(studyEntryElement);
				...
				model.status.set(StudyRevisionSummaryWidgetModel.STATUS_SUBMISSION_NOT_FOUND);
...
*/
	...
	public Deferred<Element> retrieveStudy() {
		...
		model.status.set(StudyRevisionSummaryWidgetModel.STATUS_RETRIEVE_STUDY_PENDING);
	...
	public final Function<Element, Element> retrieveStudyCallback = new Function<Element, Element>() {
		...
		public Element apply(Element in) {
			...
			model.status.set(StudyRevisionSummaryWidgetModel.STATUS_READY_FOR_INTERACTION);
	...
	public void saveStudy() {
		...
		model.status.set(StudyRevisionSummaryWidgetModel.STATUS_SAVE_STUDY_PENDING);
