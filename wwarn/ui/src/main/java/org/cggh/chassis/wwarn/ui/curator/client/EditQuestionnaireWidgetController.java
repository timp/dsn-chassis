 ...
 *
 */
public class EditQuestionnaireWidgetController {

	
	
	
	private Log log = LogFactory.getLog(EditQuestionnaireWidgetController.class);
	
	
	
	
	private EditQuestionnaireWidget owner;
	private EditQuestionnaireWidgetModel model;


	
	
	public EditQuestionnaireWidgetController(EditQuestionnaireWidget owner, EditQuestionnaireWidgetModel model) {
		this.owner = owner;
		this.model = model;
	}
	...
	private Deferred<Element> retrieveSubmission() {
		...
		model.status.set(EditQuestionnaireWidgetModel.STATUS_RETRIEVE_SUBMISSION_PENDING);
...
//				model.studyEntryElement.set(studyEntryElement);
				...
				model.status.set(EditQuestionnaireWidgetModel.STATUS_SUBMISSION_NOT_FOUND);
...
*/
	...
	public Deferred<Element> retrieveStudy() {
		...
		model.status.set(EditQuestionnaireWidgetModel.STATUS_RETRIEVE_STUDY_PENDING);
	...
	public final Function<Element, Element> retrieveStudyCallback = new Function<Element, Element>() {
		...
		public Element apply(Element in) {
			...
			model.status.set(EditQuestionnaireWidgetModel.STATUS_READY_FOR_INTERACTION);
	...
	public void saveStudy() {
		...
		model.status.set(EditQuestionnaireWidgetModel.STATUS_SAVE_STUDY_PENDING);
