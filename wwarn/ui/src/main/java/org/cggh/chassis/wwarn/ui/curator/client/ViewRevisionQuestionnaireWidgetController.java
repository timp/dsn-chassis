 ...
 *
 */
public class ViewRevisionQuestionnaireWidgetController {

	
	
	
	private Log log = LogFactory.getLog(ViewRevisionQuestionnaireWidgetController.class);
	
	
	
	
	private ViewRevisionQuestionnaireWidget owner;
	private ViewRevisionQuestionnaireWidgetModel model;


	
	
	public ViewRevisionQuestionnaireWidgetController(ViewRevisionQuestionnaireWidget owner, ViewRevisionQuestionnaireWidgetModel model) {
		this.owner = owner;
		this.model = model;
	}
	...
	private Deferred<Element> retrieveSubmission() {
		...
		model.status.set(ViewRevisionQuestionnaireWidgetModel.STATUS_RETRIEVE_SUBMISSION_PENDING);
...
//				model.studyEntryElement.set(studyEntryElement);
				...
				model.status.set(ViewRevisionQuestionnaireWidgetModel.STATUS_SUBMISSION_NOT_FOUND);
...
*/
	...
	public Deferred<Element> retrieveStudy() {
		...
		model.status.set(ViewRevisionQuestionnaireWidgetModel.STATUS_RETRIEVE_STUDY_PENDING);
	...
	public final Function<Element, Element> retrieveStudyCallback = new Function<Element, Element>() {
		...
		public Element apply(Element in) {
			...
			model.status.set(ViewRevisionQuestionnaireWidgetModel.STATUS_READY_FOR_INTERACTION);
	...
	public void saveStudy() {
		...
		model.status.set(ViewRevisionQuestionnaireWidgetModel.STATUS_SAVE_STUDY_PENDING);
