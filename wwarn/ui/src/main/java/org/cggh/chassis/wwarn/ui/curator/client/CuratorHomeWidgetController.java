...
public class CuratorHomeWidgetController {
	...
	private static final Log log = LogFactory.getLog(CuratorHomeWidgetController.class);
	 ...
	 * @param model
	 */
	public CuratorHomeWidgetController(
			CuratorHomeWidget owner,
			CuratorHomeWidgetModel model) {
	...
	public Deferred<Document> retrieveSubmissions() {
		...
		model.setStatus(CuratorHomeWidgetModel.STATUS_RETRIEVE_SUBMISSIONS_PENDING);
...
//			model.setSubmissions(submissionsFeedDoc);
			...
			model.setStatus(CuratorHomeWidgetModel.STATUS_SUBMISSIONS_RETRIEVED);
	...
	public class RetrieveSubmissionsErrback implements Function<Throwable, Throwable> {
		...
		public Throwable apply(Throwable in) {
			...
			// Update the model's status.
			model.setStatus(CuratorHomeWidgetModel.STATUS_ERROR);			
