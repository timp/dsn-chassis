...
declare function local:content($content) as item()*
{
    ...
    let $drafts-v1-0-1 := local:get-old-versioned-content-drafts($drafts) 
    let $drafts-v1-1 := local:get-new-versioned-content-drafts($drafts)    
    
    return
    
        <html>
            <head>
                <title>Data Migration - Draft v1.0.1 to v1.1</title>
            </head>
            <body>
                <h1>Data Migration - Draft v1.0.1 to v1.1</h1>
  
                <p>This script will:</p>
                
                <ul>
                    <li>Add a curator-notes element to all study entries, after the acknowledgements element.</li>
                    <li>Add a study-status element with the value "new" (Newly created study) to all study entries, after the curator-notes element.</li>
                </ul>
                ...
                <p>Number of entries in <a href="../content/drafts">Draft</a> collection using v1.1 profile: <strong>{ count( $drafts-v1-1 ) }</strong></p>                
...
declare function local:do-modifications-drafts($drafts-v1-0-1) as element( atom:entry )*
{
    let $new := local:modify-nodes($drafts-v1-0-1)
...
declare function local:modify-nodes($collection-v1-0-1) as element( atom:entry )*
{
   ...
   let $collection-v1-1 := 
       ...
       return update insert <study-status>new</study-status> following $v1-0-1//study/curator-notes
...
return 
    ...
    else common-protocol:do-method-not-allowed( "/admin/migrate-draft-1.0.1-to-1.1.xql" , "/admin/migrate-draft-1.0.1-to-1.1.xql" , ( "GET" , "POST" ) )
