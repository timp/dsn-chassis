...
<xsl:stylesheet version="1.0" xmlns:xsl="http://www.w3.org/1999/XSL/Transform"
	...
	<xsl:variable name="clinical.regimen.drug.activeIngredients" select="3"/>
	<xsl:variable name="clinical.regimen.drug.activeIngredients.colDef"
		select="'activeIngredientName,activeIngredientMgPerDose,'"/>
	<xsl:variable name="clinical.regimen.drug.ageDosing" select="3"/>
	<xsl:variable name="clinical.regimen.drug.ageDosing.colDef"
		select="'day,hour,ageFrom,ageTo,dose,doseUnit,'"/>
	<xsl:variable name="clinical.regimen.drug.weightGroupDosing" select="3"/>
	<xsl:variable name="clinical.regimen.drug.weightGroupDosing.colDef"
		select="'day,hour,wGroupFrom,wGroupTo,dose,doseUnit,'"/>
	<xsl:variable name="clinical.regimen.drug.weightDosing" select="3"/>
	<xsl:variable name="clinical.regimen.drug.weightDosing.colDef" select="'day,hour,dose,'"/>
	...
	<xsl:template match="atom:content">
		<xsl:text>SSQ-version,xslt-version,</xsl:text>
...
-->
	...
	<xsl:template match="sites/site">
		...
		<xsl:for-each select="*">
			<xsl:if test="not((name(.) = 'transmissionIntensity') or name(.) = 'drugs')">
				<xsl:choose>
					<xsl:when test="$outputType = 1">
						<xsl:value-of select="normalize-space(.)"/>
	...
	<xsl:template match="clinical">
		...
		<xsl:apply-templates select="geneotypingToDistinguishBetweenRecrudescenceAndReinfection">
			<xsl:with-param name="outputType" select="$outputType"/>
		</xsl:apply-templates>
		<xsl:apply-templates
	...
	<xsl:template name="addDrugs">
		...
		<xsl:if test="$count &lt;= $total">
			...
			<xsl:call-template name="addDrugInfo">
				<xsl:with-param name="outputType" select="$outputType"/>
				<xsl:with-param name="total" select="$clinical.regimen.drug.activeIngredients"/>
				<xsl:with-param name="colDef"
					select="$clinical.regimen.drug.activeIngredients.colDef"/>
				<xsl:with-param name="prefix" select="concat($newpref,'activeIngredient')"/>
				<xsl:with-param name="count" select="1"/>
			</xsl:call-template>

			<xsl:call-template name="addDrugInfo">
				<xsl:with-param name="outputType" select="$outputType"/>
				<xsl:with-param name="total" select="$clinical.regimen.drug.ageDosing"/>
				<xsl:with-param name="colDef" select="$clinical.regimen.drug.ageDosing.colDef"/>
				<xsl:with-param name="prefix" select="concat($newpref,'ageDosingSchedule')"/>
				<xsl:with-param name="count" select="1"/>
			</xsl:call-template>

			<xsl:call-template name="addDrugInfo">
				<xsl:with-param name="outputType" select="$outputType"/>
				<xsl:with-param name="total" select="$clinical.regimen.drug.weightGroupDosing"/>
				<xsl:with-param name="colDef"
					select="$clinical.regimen.drug.weightGroupDosing.colDef"/>
				<xsl:with-param name="prefix" select="concat($newpref,'wGroupDosingSchedule')"/>
				<xsl:with-param name="count" select="1"/>
			</xsl:call-template>

			<xsl:call-template name="addDrugInfo">
				<xsl:with-param name="outputType" select="$outputType"/>
				<xsl:with-param name="total" select="$clinical.regimen.drug.weightDosing"/>
				<xsl:with-param name="colDef" select="$clinical.regimen.drug.weightDosing.colDef"/>
				<xsl:with-param name="prefix" select="concat($newpref,'weightDosingSchedule')"/>
				<xsl:with-param name="count" select="1"/>
			</xsl:call-template>
			<xsl:call-template name="addDrugs">
				<xsl:with-param name="count" select="$count + 1"/>
	...
	<xsl:template match="drugs/drug">
		...
		<xsl:call-template name="addDrugInfo">
			<xsl:with-param name="outputType" select="$outputType"/>
			<xsl:with-param name="total" select="$clinical.regimen.drug.activeIngredients"/>
			<xsl:with-param name="colDef" select="$clinical.regimen.drug.activeIngredients.colDef"/>
			<xsl:with-param name="prefix" select="concat($newpref,'activeIngredient')"/>
			<xsl:with-param name="count" select="count(activeIngredients/activeIngredient) + 1"/>
		</xsl:call-template>
		...
		<xsl:call-template name="addDrugInfo">
			...
			<xsl:with-param name="colDef" select="$clinical.regimen.drug.ageDosing.colDef"/>
			<xsl:with-param name="prefix" select="concat($newpref,'ageDosingSchedule')"/>
			<xsl:with-param name="count" select="count(ageDosing/ageDosingSchedule) + 1"/>
		</xsl:call-template>
		...
		<xsl:call-template name="addDrugInfo">
			<xsl:with-param name="outputType" select="$outputType"/>
			<xsl:with-param name="total" select="$clinical.regimen.drug.weightGroupDosing"/>
			<xsl:with-param name="colDef" select="$clinical.regimen.drug.weightGroupDosing.colDef"/>
			<xsl:with-param name="prefix" select="concat($newpref,'wGroupDosingSchedule')"/>
			<xsl:with-param name="count" select="count(wGroupDosing/wGroupDosingSchedule) + 1"/>
		</xsl:call-template>
		...
		<xsl:call-template name="addDrugInfo">
			...
			<xsl:with-param name="colDef" select="$clinical.regimen.drug.weightDosing.colDef"/>
			<xsl:with-param name="prefix" select="concat($newpref,'weightDosingSchedule')"/>
			<xsl:with-param name="count" select="count(weightDosing/weightDosingSchedule) + 1"/>
		</xsl:call-template>
			...
			<xsl:when test="contains($dataString,',')">
				...
				<xsl:choose>
					<xsl:when test="$outputType = 0">
						...
						<xsl:value-of select="concat($parentNum,substring-before($dataString,','))"
						/>
					</xsl:when>
				</xsl:choose>
				<xsl:value-of select="','"/>
	...
	<xsl:template match="geneotypingToDistinguishBetweenRecrudescenceAndReinfection">
		<xsl:param name="outputType"/>
	...
	<xsl:template match="applicable">
		<xsl:param name="outputType"/>
		<xsl:for-each select="genotypingLaboratory|markerDiscriminant|markerDiscriminantOther">
			<xsl:choose>
				<xsl:when test="$outputType = 1">
					<xsl:value-of select="normalize-space(.)"/>
				</xsl:when>
	...
	<xsl:template name="addSample">
		...
		<xsl:if test="$count &lt;= $total">
			...
			<xsl:call-template name="addBlanks">
				...
				<xsl:with-param name="prefix" select="concat($prefix,$count,'.storage')"/>
			</xsl:call-template>
	...
	<xsl:template match="analytes/analyte">
		...
		<xsl:for-each select="*">
			<xsl:choose>
				...
				<xsl:otherwise>
					<xsl:value-of select="concat($prefix,1 + count(parent::*/preceding-sibling::*))"/>
					<xsl:value-of select="'.'"/>
					<xsl:value-of select="name(.)"/>
					<xsl:value-of select="','"/>
				</xsl:otherwise>
	...
	<xsl:template match="assayReferences/assayReference">
		...
		<xsl:call-template name="addBlanks">
			<xsl:with-param name="count" select="count(parent::node()//assayReference)"/>
			<xsl:with-param name="outputType" select="$outputType"/>
			<xsl:with-param name="colDef" select="$pharmacology.assayReferences.colDef"/>
